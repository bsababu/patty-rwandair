...
class AppController {
  ...
  @Get("ready") async ready() {
    try {
      ...
      throw new BadRequestException("Dependencies unavailable");
    }
  }
  ...
  @Post("v1/auth/login") async login(
    ...
    await redis.set(
      "session:" + sessionId,
      JSON.stringify({ user: safe, csrf, sessionVersion: user.sessionVersion }),
      "EX",
      SESSION_SECONDS,
    );
...
async function bootstrap() {
  await redis.connect().catch((e) => {
    ...
    throw new Error(
      `Redis is unavailable at ${process.env.REDIS_URL || "redis://redis:6379"}. Start Redis before the API.`,
    );
  });
