...
redis.on("error", () => {
    // Startup reports the actionable dependency error below.
});
...
async function authenticate(req, roles) {
    ...
    await redis.expire("session:" + sessionId, SESSION_SECONDS);
    return user;
}
...
let AppController = class AppController {
    ...
    async ready() {
        try {
            ...
            throw new common_1.BadRequestException("Dependencies unavailable");
        }
    }
    async login(body, req, res) {
        ...
        await redis.set("session:" + sessionId, JSON.stringify({ user: safe, csrf, sessionVersion: user.sessionVersion }), "EX", SESSION_SECONDS);
...
async function bootstrap() {
    ...
    await redis.connect().catch((e) => {
        if (String(e).includes("already connecting"))
            return;
        throw new Error(`Redis is unavailable at ${process.env.REDIS_URL || "redis://redis:6379"}. Start Redis before the API.`);
    });
